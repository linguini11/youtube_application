package com.google;

import java.io.File;
import java.io.FileNotFoundException;
import java.util.ArrayList;
import java.util.Arrays;
import java.util.HashMap;
import java.util.List;
import java.util.Scanner;
import java.util.stream.Collectors;
import java.util.Set;

/**
 * A class used to represent a Video Library.
 */
class VideoLibrary {

  private final HashMap<String, Video> videos;
  private HashMap<String, String> flagMap;

  VideoLibrary() {
    this.videos = new HashMap<>();
    this.flagMap = new HashMap<>();
    try {
      File file = new File(this.getClass().getResource("/videos.txt").getFile());

      Scanner scanner = new Scanner(file);
      while (scanner.hasNextLine()) {
        String line = scanner.nextLine();
        String[] split = line.split("\\|");
        String title = split[0].strip();
        String id = split[1].strip();
        List<String> tags;
        if (split.length > 2) {
          tags = Arrays.stream(split[2].split(",")).map(String::strip).collect(
              Collectors.toList());
        } else {
          tags = new ArrayList<>();
        }
        this.videos.put(id, new Video(title, id, tags));
      }
    } catch (FileNotFoundException e) {
      System.out.println("Couldn't find videos.txt");
      e.printStackTrace();
    }
  }

  List<Video> getVideos() {
    return new ArrayList<>(this.videos.values());
  }

  /**
   * Get a video by id. Returns null if the video is not found.
   */
  Video getVideo(String videoId) {
    return this.videos.get(videoId);
  }

  public void addFlag(String videoId, String reason) {
    flagMap.put(videoId, reason);
  }
  
  public Boolean isVideoFlagged(String videoId) {
    return flagMap.containsKey(videoId);
  }

  public void removeFlag(String videoId) {
    flagMap.remove(videoId);
  }

  public String getFlagReason(String videoId) {
    return flagMap.get(videoId);
  }

  public List<Video> getUnflaggedVideos() {
    List<Video> unflaggedVideos = new ArrayList<Video>();
    List<String> videoKeys = new ArrayList<>(videos.keySet());
    List<String> flagKeys = new ArrayList<>(flagMap.keySet());
    for (int i = 0; i < videoKeys.size(); i++) {
      if (!flagKeys.contains(videoKeys.get(i))) {
        unflaggedVideos.add(getVideo(videoKeys.get(i)));
      }
    }
   return unflaggedVideos;
  }

}
